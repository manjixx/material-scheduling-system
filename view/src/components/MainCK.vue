<template>
  <div>
   <el-card >
     <el-row class="padding-xs" type="flex">
       <el-col :span="12" class="padding-xs">
        <div id="myBar"  class="echarts-view-bar"></div>
       </el-col>
      <el-col :span="12" class="padding-xs">
         <div>
            <el-input
    ...
    <el-dialog
      ...
      <el-form ref="createForm" :model="healthData"  :rules="healthRules" label-width="80px">
        <el-form-item label="用户名"  prop="healthsName">
        ...
        <el-form-item label="所在部门" prop="healthsJob">
          <el-select v-model="healthData.healthsJob">
            ...
            <el-option label="安管" value="安管"></el-option>
...
<script>
import { getHealths, createHealths, getStores } from './../api/index';
import {
...
export default {
  ...
  created() {
    ...
    if (userInfo) {
      ...
      this.healthData.healthsName = userinfo.username;
      this.getStores(userinfo.id);
    }
    this.getHealthsList();
  },
  mounted(){
 ...
 //输入框
    ...
    initBar(){
      ...
      let option = {
        ...
        xAxis: {
            type: 'category',
            data: ['口罩', '体温计', '防护服', '酒精', '肩背式喷雾器', '一次性手套', '84消毒液']
        },
        yAxis: {
            type: 'value'
        },
