...
const adminChildren = [
    ...
    },
    {
      path: '/mainck',
      name: 'MainCK',
      component: () => import(/* webpackChunkName: "main" */ '../components/MainCK')
    },
    {
      path: '/mainus',
      name: 'MainUS',
      component: () => import(/* webpackChunkName: "main" */ '../components/MainUS')
    },
    {
      path: '/users',
      name: 'Users',
    ...
    },
    {
      path: '/goodsCK',
      name: 'GoodsCK',
      component: () => import(/* webpackChunkName: "goods_out" */ '../components/GoodsCK')
    },
    {
      path: '/order',
      name: 'Order',
      component: () => import(/* webpackChunkName: "goods_out" */ '../components/Order')
    }
];
let mainview = '';
...
router.beforeEach((to, from, next) => {
  ...
  if(to.path == '/admin'){
    ...
    if(userInfo.roles[0].id == 1){
      routes[4].redirect = 'main';
    }
    else if(userInfo.roles[0].id == 2){
      routes[4].redirect = 'mainck';
    }
    else if(userInfo.roles[0].id == 3){
      routes[4].redirect = 'mainus';
    }
    console.log('after: ', to.path);
    return next(routes[4].redirect);
  }
...
const getRouterRights = () => {
  ...
  const routerRights = userInfo.rightsTree.filter((rights) => {
    if (rights && rights.rightsType === 'router') return rights;
  });
